...
def get_fund_k_history(fund_code: str, pz: int = 30) -> pd.DataFrame:
    ...
    data = {}
    fund_code=931151
    code_url = 'https://xueqiu.com/query/v1/suggest_stock.json?q={secid}'
    url='https://stock.xueqiu.com/v5/stock/chart/kline.json?symbol={code}&begin=1691974773318&period=day&type=before&count=-284&indicator=kline,pe,pb,ps,pcf,market_capital,agt,ggt,balance'
    secid_value = f'{fund_code}'
    code_url = code_url.replace('{secid}', secid_value)
    url = url.replace('{code}', secid_value)
    # print('fund_code'+fund_code)
    # 转换基金编码
    requests.get(code_url, headers=EastmoneyFundHeaders, data=data).json()
